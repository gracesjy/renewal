...
## 기본 검정: 정규분포(Normality Test) 관련된 테스트
Anderson-Darling Test와 Shapiro-Wilk Test는 둘 다 정규성 검정(Normality Test) 목적으로 서로를 완전히 대체하여 사용할 수 있다.

| 구분 | Shapiro-Wilk Test | Anderson-Darling Test |
| :--- | :--- | :--- |
| **주요 용도** | **정규성 전용 검정** | **정규성 + 다양한 분포 적합도 검정** |
| **통계적 검정력 (Power)** | **소/중규모 표본에서 가장 뛰어남** | 표본 크기와 무관하게 전반적으로 우수 |
| **특징적 강점** | 데이터 중심부 및 전반적인 분포 왜곡 감지에 민감 | **분포의 양쪽 꼬리(Tail) 영역 이상치 감지에 특화** |
| **권장 표본 크기** | $n < 50$ 또는 $n < 2,000$ (소/중규모) | $n \ge 50$ 이상 (중/대규모) |
| **범용성** | 오직 **정규분포** 검정에만 적용 가능 | 정규분포 외 지수분포, 웨이블 분포 등 확장 가능 |
| **주요 활용 분야** | 일반 학술 연구, R/Python 기본 데이터 분석 | 품질 관리(6 Sigma), 신뢰성 공학, Minitab 표준 |

Shapiro-Wilk (또는 Anderson-Darling) 검정 결과:
   > $p \ge 0.05$ (정규성 만족) $\rightarrow$ 검정력이 우수한 Bartlett's Test 적용
   > $p < 0.05$ (정규성 미만족) $\rightarrow$ 비정규성에 강한 Levene's Test 적용

## 기본 검정: 등분산성(Homogeneity of variance) 검정

| 구분 | Bartlett's Test | Levene's Test |
| :--- | :--- | :--- |
| **주요 용도** | **모수적 등분산성 검정** | **비모수/건실(Robust) 등분산성 검정** |
| **정규성 가정 필수 여부** | **매우 엄격함 (Mandatory)** | **필요 없음 (Robust to non-normality)** |
| **통계적 검정력 (Power)** | 정규분포 만족 시 **최고의 검정력** | 정규분포 만족 시 Bartlett 대비 약간 낮음 |
| **민감도 (Sensitivity)** | 정규성 위배 시 등분산성이 맞아도 1종 오류(거짓 긍정) 발생 | 이상치(Outliers) 및 비정규성 분포 데이터에 안정적 |
| **연산 방식** | 집단별 표본 분산의 대수(Log) 및 가중평균 기반 | 각 데이터와 **중앙값(Median)** 또는 평균 간 절대 편차 기반 |
| **주요 추천 상황** | Shapiro-Wilk 등을 통해 **정규성이 확실히 검증된 경우** | **정규성을 만족하지 않거나** 분포 형태를 모르는 경우 |
